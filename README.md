...
- 题材纠偏：同时保留原始概念题材、重分类题材、主导催化、匹配度和偏差原因，优先回答“今天跟哪条事件主线涨”。
...
系统会在日报中输出高位强势股的强转弱观察字段：

```text
强转弱阶段、强转弱风险分、强转弱信号、观察纪律
```

这些字段用于把“高位换手、修复失败、相对题材偏弱、放量滞涨、跌破关键均线”等信号沉淀为盘后纪律，不等同于自动买卖建议。
...
```text
SH.600001,示例股票,AI算力,核心持仓
```

观察名单股票会并入每日复盘目标池；如果当天快照中没有该股票，系统会自动跳过，不影响日报生成。
...
HTML 报告会同步总结当日涨停板块结构、板块研究候选、题材纠偏审计、连板梯队、成交额核心、近期变化趋势，并在可联网且公开可访问时检索雪球及类似财经论坛。论坛内容会标注为市场讨论，不能替代公告、新闻、龙虎榜等证据。
...
`核心题材` 现在默认采用主题库或 Futu 分类结果；`原始题材` 保留同一分类来源下的首个题材标签，`重分类题材`/`主导催化`/`题材匹配度`/`偏差原因` 继续用于展示事件规则对题材的辅助判断。`成交额(亿元)` 仅用于日报展示，内部计算仍使用原始元单位；`所属行业` 会优先展示主题库的父级行业，例如 `半导体`。

`所属行业`、`所属概念`、`原始题材`、`核心题材` 的分类来源按以下顺序处理：

1. 优先读取本机 `A股主题研究中枢/data/theme_hub.sqlite3` 中已发布的股票—主题关系；主题的父级作为所属行业，主题本身作为所属概念、原始题材和核心题材。
2. 如果主题库没有该股票的已发布分类，则四个字段均使用 Futu 的行业/概念分类，并在 `题材分类来源` 中标记为 `Futu分类（A股主题库无相应分类）`。

这样可以区分“主题库正式分类”和“Futu 临时兜底分类”，不会把 Futu 的概念标签误当成本机主题库结论。
